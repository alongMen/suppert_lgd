<template>
  <div class="withdraw-record user-center-main-area">
    <h4 class="recharge-title">用户列表</h4>
    <p class="tils">查看用户</p>
    ...
    <Table
      ...
      :data="data"
    >
      <template slot="memberType" slot-scope="{ row }">
          <span class='status-ing' v-if='row.memberType===1'>会员</span>
          <span class='status-success' v-if='row.memberType===2'>代理</span>
      </template>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      query.memberType = this.form.memberType;
