<template>
  <div class="board user-center-main-area">
    ...
    <ul class="list" v-else>
      <li class="borad-item" v-for="(item, index) in data" :key="index" @click="detail(item)">
        <div class="date">
    ...
    <my-pagination :page="page" :limit="limit" :total="total" @change="pageChange" style="margin-top:30px;" v-if="total > limit" />
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      const res = await get_board_list(this.page, this.limit);
      this.loading = false;
      if (res.status < 400) {
