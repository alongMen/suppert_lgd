        ...
        <div class="input-amount">
          ...
          <span class="info" style="margin-left:17px;"
            >单次提现金额：<span class="number"
              >{{ parsePrice(info.singleMin) }} ~
              {{ parsePrice(info.singleMax) }}</span
            >
            元</span
          >
        </div>
...
export default {
  ...
  computed: {
    ...
    balance() {
      return this.$store.state.base.user_info.amount;
    },
    fee() {
    ...
    costFlow() {
      let costFlow = 0;
      const amount = this.amount;
      const { memberFlow } = this.info;
      costFlow = memberFlow * amount;
      return costFlow;
    },
    adminFee() {
  ...
  methods: {
    ...
    ok() {
      ...
      if (amount < singleMin || amount > singleMax) {
        this.$Message.error(`提款金额为${singleMin} ~ ${singleMax}元`);
